...
def answer_question(question, index, embedding_model, reranker):
    ...
    if raw and raw.startswith("__LLM_ERROR__"):
        # سيب الرسالة تظهر في Streamlit Cloud logs عشان تعرف السبب الحقيقي
        # (rate limit، موديل متوقف، مفتاح غلط...) بدل ما تختفي بصمت.
        print(f"[APP] LLM call failed: {raw}", flush=True)
